...
function ModalPopupWindow() {
    ...
    function ResizePopUp(height, width) {
        ...
        var left = (window.screen.availWidth - width) / 2;       // last changes 2 to 1.4
        var top = (window.screen.availHeight - height) / 2;     // last change 2 to 1.4
        var xy = GetScroll();
        if (maximize) {
            left = xy[0] + 10;
            top = xy[1] + 10;
    ...
    function __InitModalPopUp(height, width, title, from_top, from_left) {
        orginalWidth = width;
        ...
        var tdOverLay = document.getElementById("bailwal_td_overlay");
        var left = (window.screen.availWidth - width) / 2;         // last changes 2 to 1.4
        var top = (window.screen.availHeight - height) / 2;        // last changes 2 to 1.4
        var xy = GetScroll();
        left += xy[0];
        top += xy[1];
        ...
        divFrameParent.style.top = from_top + "px";          // last changes as top to 60 
        divFrameParent.style.left = from_left + "px";        // last changes as left to 270
        divFrameParent.style.height = height + "px";
        divFrameParent.style.width = width + "px";
    ...
    this.ShowURL = function (url, height, width, title, onCloseCallBack, callbackFunctionArray, maxmizeBtn, from_top, from_left) {
        __InitModalPopUp(height, width, title, from_top, from_left);
        var divFrameParent = document.getElementById("bailwal_div_frame_parent");
        var divOverlay = document.getElementById("bailwal_div_overlay");
        var iframe = document.getElementById("bailwal_overlay_frame");
        var tdOverLay = document.getElementById("bailwal_td_overlay");
    ...
    this.ShowMessage = function (message, height, width, title, from_top, from_left) {
        __InitModalPopUp(height, width, title, from_top, from_left);
    ...
    this.ShowConfirmationMessage = function (message, height, width, title, onCloseCallBack, firstButtonText, onFirstButtonClick, secondButtonText, onSecondButtonClick, from_top, from_left) {
        this.ShowMessage(message, height, width, title, from_top, from_left);
        var tdOverLay = document.getElementById("bailwal_td_overlay");
        var maxWidth = 100;
        document.getElementById("bailwal_span_message").innerHTML = message;
        document.getElementById("bailwal_div_message").style.display = "";
